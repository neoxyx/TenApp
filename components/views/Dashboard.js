...
export class Dashboard extends Component {
	...
	render() {
		...
		const Check = (props) => {
			return <CheckBox checked={props.value} onPress={() => this.setState({ check: !props.value })} />;
		};
		...
		const items = this.state.details.map((item, j) => {
			...
			return (
				...
				>
					<Icon style={{ marginLeft: 10 }} name="handshake-o" size={40} type="font-awesome" color="#399998" />
					{item.activity_result_type == 'bool' && (
						<>
							...
							<Check
								onValueChange={(value) => setProtocolDetailAnswerValue(j, value)}
								value={Value}
								id={item.id}
							/>
						</>
									...
									onPress={() =>
										alert(
											'Save data: ' +
												...
												' ' +
												this.state.arrivalTimeHour +
												':' +
												this.state.arrivalTimeMin +
												' Notes: ' +
												this.state.notes
										)
									}
